...
// int8-native variant: the latent KV cache is stored group-quantized
// (packed uint32 + per-group scales/biases). Instead of receiving a dense
// fp16/bf16 latent, this kernel dequantizes only the top-k rows it gathers,
// directly into the existing KVs staging tile. Halves latent KV read
// bandwidth (sparse MLA is bandwidth-bound) and removes the full-dense
// dequant transient. 8-bit affine only (512 = 8 groups of GS=64): one byte
// per latent element, one (scale,bias) per (row, group). Apple GPUs have no
// int8 MMA, so dequant -> fp16/bf16 tile -> existing fp16 MMA path.
template <typename T, int GS>
METAL_FUNC T glm_dsa_dequant_latent_elem(
    const device uint8_t* packed_row, // byte view of this row's packed latent
    const device T* scales_row,
    const device T* biases_row,
    int e) { // latent element index in [0, D_LATENT)
  // T arithmetic to match mx.dequantize(...) bit-for-bit (output dtype = T).
  return scales_row[e / GS] * static_cast<T>(packed_row[e]) + biases_row[e / GS];
}
...
template <
    ...
    typename IndexT,
    typename AccumType = float>
[[kernel, max_total_threads_per_threadgroup(WM * 32)]] void sparse_mla_attention_q8(
    const device T* Q_latent [[buffer(0)]],
    const device T* Q_pe [[buffer(1)]],
    const device uint32_t* KV_packed [[buffer(2)]],
    const device T* KV_scales [[buffer(3)]],
    const device T* KV_biases [[buffer(4)]],
    const device T* K_pe [[buffer(5)]],
    const device IndexT* Topk [[buffer(6)]],
    const device uint* TopkLength [[buffer(7)]],
    device T* O [[buffer(8)]],
    const constant GlmDsaSparseMlaQ8Params* params [[buffer(9)]],
    uint simd_lane_id [[thread_index_in_simdgroup]],
    uint simd_group_id [[simdgroup_index_in_threadgroup]],
    uint3 tid [[threadgroup_position_in_grid]]) { // clang-format on

  static_assert(D_LATENT % GS == 0, "Latent dim must divide group size.");
  ...
  const device T* q_pe_base = Q_pe + size_t(b) * params->Q_pe_strides[0] +
      size_t(q_pos) * params->Q_pe_strides[2];
  const device uint32_t* kv_packed_base =
      KV_packed + size_t(b) * params->KV_packed_strides[0];
  const device T* kv_scales_base =
      KV_scales + size_t(b) * params->KV_scales_strides[0];
  const device T* kv_biases_base =
      KV_biases + size_t(b) * params->KV_biases_strides[0];
  const device T* k_pe_base = K_pe + size_t(b) * params->K_pe_strides[0];
  const bool prefix_row = params->causal_prefix_indices &&
      params->topk_valid_prefix && q_pos < params->causal_prefix_rows;
  const int topk_row = prefix_row ? 0 : q_pos - params->causal_prefix_rows;
  ...
  for (int ktile = 0; ktile < n_tiles; ++ktile) {
    ...
    for (short dchunk = 0; dchunk < D_CHUNKS; ++dchunk) {
      ...
      for (int elem = lane; elem < BK * DC; elem += tgp_size) {
        ...
        if (k_pos >= 0) {
          if (dbase < D_LATENT) {
            const device uint8_t* prow = reinterpret_cast<const device uint8_t*>(
                kv_packed_base + size_t(k_pos) * params->KV_packed_strides[2]);
            value = glm_dsa_dequant_latent_elem<T, GS>(
                prow,
                kv_scales_base + size_t(k_pos) * params->KV_scales_strides[2],
                kv_biases_base + size_t(k_pos) * params->KV_biases_strides[2],
                dbase + d);
          } else {
            value = k_pe_base[size_t(k_pos) * params->K_pe_strides[2] +
                              (dbase - D_LATENT) + d];
          }
        }
        KVs[k + d * LDK] = value;
